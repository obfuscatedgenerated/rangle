...
export const AccountFlyout = ({open, on_close}: AccountFlyoutProps) => {
    ...
    return (
        <>
            ...
            <div aria-hidden={!open} className={`pt-[calc(var(--sait)+calc(var(--spacing)*4))] pl-6 sm:pl-4 p-4 z-9999 fixed top-0 right-0 w-100 h-full bg-background-variant transition-opacity sm:transition-all duration-500 sm:transform ${open ? "sm:translate-x-0 opacity-100" : "sm:translate-x-full opacity-0 pointer-events-none"}`}>
