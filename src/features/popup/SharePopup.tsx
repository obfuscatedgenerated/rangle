...
export const SharePopup = ({open, on_close, attempts, today_data, archive_date, hardcore, bonus_results, open_leaderboard}: SharePopupProps) => {
    ...
    return (
        <dialog onAbort={on_close} ref={dialog_ref}
            ...
            {in_discord_guild && (
                <span className="underline cursor-pointer font-bold flex items-center justify-center gap-2" onClick={open_leaderboard}><Crown className="h-3 w-3" /> View server leaderboard</span>
            )}
